...
## Queries
### Complex
1. C- Query 1: List the total billing amount for each patient along with the number of payments they have made.
...
2. C- Query 2: List the storage facilities that are nearing capacity.
...
3. C- Query 3: List Patients and Payment Details if they have made more than one payment.
...
4. C- Query 4: List the patient's name and ID number who are identified as High-Risk Patients (Unpaid Bills + Frequent Visits). To be considered High-Risk, patients have to have both 3+ unpaid bills and 4+ visits.
...
5. C- Query 5: List Patients name, ID, visit date, and visit ID if they have overlapping two or more visits on the same day.
...
6. C- Query 6: Retrieve the latest prescription for each patient and list the medicine name, dosage amount and frequency of the medicine
...
10. S- Query 2: Count the number of insurance claims filed by each patient.
